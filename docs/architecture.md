...
ANTLR 只负责识别语法并产生平台无关 AST；Temporal evaluator 负责日期糖、当前时间与时长糖、未知时间、时区、重复规则、`by[...]`、排除和实例展开。

时间位置支持 `now`，以及小于 24 小时的单一整数时长 `h`/`m`。`10:00-2h` 使用显式起点，单独的 `2h` 以 `now` 为起点；时长按墙钟时间计算。小时已知而分钟未知时，只有整小时长度可以确定地传播未知分钟，例如 `10:?-2h` 规范化为 `10:?-12:?`。保存前必须把 `now` 和时长展开为明确的 `HH:mm` 起止时间。
...
- 用户可配置初始为空的时区简写映射；简写不区分大小写，解析后必须固化为完整 IANA 时区。
