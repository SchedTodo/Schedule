...
describe('secondary pages', () => {
  ...
  it('restores the Schedule header with Info and Times cards', async () => {
    ...
    expect(wrapper.findAll('.recurrence-code .cm-line').map((line) => line.text()))
      .toEqual(['2026/7/15 10:00;', '2026/7/16 11:00;', ''])
    expect(wrapper.findAll('.exclusion-code .cm-line').map((line) => line.text()))
      .toEqual(['2026/7/17 12:00;', '2026/7/18 13:00;', ''])
